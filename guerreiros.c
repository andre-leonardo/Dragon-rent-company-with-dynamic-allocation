...
int salvarGuerreiro(Guerreiro warrior)
{
	...
	if (guerreiro != NULL)
	{
		...
		for(i = 0; i < ARRSIZEGUERREIRO; i++)
		{
			if(guerreiro[i].codigo > codigoAtualGuerreiros)//o código deve ser gerado dentro da função salvarGuerreiro. a quantidade de guerreiros não consegue gerar um código válido sempre. ele pode gerar guerreiros com códigos iguais.
